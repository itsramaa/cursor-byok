...
func New(addr string, ca *certs.CA, gw *relay.Gateway, resolver AgentResolver, selectedModel func(string) string) (*Server, error) {
	...
	// Only MITM Cursor hosts. Any other HTTPS CONNECT (e.g. browser, pip, npm)
	// gets rejected so it fails fast and never causes WRONG_VERSION_NUMBER errors
	// from apps that happen to route through the WinINET system proxy.
